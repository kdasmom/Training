...
class SearchByPropertySelect extends Select {
	public function __construct($asp_client_id, $properties = [], $order = 'WF.wfrule_name') {
		parent::__construct();
		...
		$this->distinct()
			...
			->from(['WF' => 'wfrule'])
				...
				->join(new join\WFRuleWFRuleTargetJoin([], Select::JOIN_INNER, 'WFT', 'WF'))
		...
		if (count($properties)) {
			$where->in('WFT.tablekey_id', implode(',', $properties));
		}

		$where->equals('WFT.table_name', "'property'")
