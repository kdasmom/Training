...
class SearchByUserSelect01 extends Select {
    public function __construct($asp_client_id, $users = []) {
        ...
        $where = Where::get()
            ->in('WA.wfaction_originator_tablekey_id', $users)
            ->equals('WA.wfaction_originator_tablename', 'userprofilerole')
...
INNER JOIN wfaction WA ON WF.wfrule_id = WA.wfrule_id
                ...
                INNER JOIN @linktable lnk ON lnk.MyVal = WA.wfaction_originator_tablekey_id AND WA.wfaction_originator_tablename = 'userprofilerole'
