...
class SearchByRoleSelect01 extends Select {
    public function __construct($asp_client_id, $roles = []) {
        ...
        $where = Where::get()
            ->in('WA.wfaction_originator_tablekey_id', $roles)
...
FROM wfrule WF
    ...
    INNER JOIN @linktable lnk ON lnk.MyVal = WA.wfaction_originator_tablekey_id AND WA.wfaction_originator_tablename = 'role'
