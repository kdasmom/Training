...
//		}
	...
	listDiff: function(savedItems) {
		var me = this,
			defList = me.getDefaultList(),
			assignedObjects = {},
			isInArray,
			newList = [];


		for (var index = 0; index < defList.length; index++) {
			isInArray = false;
			Ext.each(savedItems, function(savedItem){
				if (defList[index][1] == savedItem) {
					assignedObjects[savedItem] = defList[index];
					isInArray = true;
					return false;
				}
			});
			if (!isInArray) {
				newList.push(defList[index]);
			}
		}

		me.data = newList;
		me.getStore().getProxy().data = me.data;
		me.getStore().reload();
